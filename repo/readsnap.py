...
def readsnap(sdir,snum,ptype,
    ...
    header_only=0,loud=0,readags=False):
    ...
    for i_file in range(numfiles):
        ...
        if(npart[ptype]>0):
            ...
            if (readags) :
                ags[nL:nR]=input_struct[bname+"AGS-Softening"]
            if (ptype==0):
    ...
    print("readags",readags)
    file.close();
    if (readags) :
        if (ptype==0):
            return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'u':ugas,'rho':rho,'h':hsml,'ne':nume,'nh':numh,'sfr':sfr,'z':metal,'ags':ags};
        if (ptype==4):
            return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'z':metal,'age':stellage,'ags':ags};
        if (ptype==5) and (skip_bh==0):
            return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'mbh':bhmass,'mdot':bhmdot}
        return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'ags':ags};
    else :
        if (ptype==0):
